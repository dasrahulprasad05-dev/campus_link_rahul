...
// Resume Matching
test('resume matching produces explainable results', async () => {
  const result = await matchResumeToJD('SQL Python Power BI communication data analysis');
...
test('resume matching handles empty input', async () => {
...
// Skill Gap
test('skill gap identifies missing skills', async () => {
  const result = await analyzeSkillGap('Data Analyst', ['SQL', 'Python']);
...
test('skill gap handles unknown role', async () => {
  const result = await analyzeSkillGap('Unknown Role', []);
  assert.ok(result.missing.length > 0, 'Should fallback to default skills');
});

// Readiness
test('readiness score is between 0 and 100', async () => {
  const result = await calculateReadiness({ skills: ['SQL'], projects: [{}], cgpa: 8.0 });
...
// Interview
test('interview evaluates short answers differently than detailed ones', async () => {
  const short = await evaluateAnswer('I used data.');
  const detailed = await evaluateAnswer('In my previous project, I analyzed sales data using SQL queries to identify declining product categories. I created a dashboard in Power BI that helped the team reduce forecast error by 15%. The situation was challenging because we had missing data in 20% of records.');
